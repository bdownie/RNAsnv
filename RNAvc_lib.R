...
train.RNAvc <- function (train,minimumCumulativeImportance=100,dist="bernoulli",save.model=NULL,cores=5,save.glm=NULL,class="AI",max.trees=50000) {
...
predict.RNAvc <- function (gbm.model,df,ntree=NULL,class="AI",decision.boundry=0.5) {
	library(gbm)
	#rm(df.tmp)
	...
	if (class == "error") { 
		colnames(df)[which(colnames(df) == "Predicted")] <- "RNAvcIsGood"
        	colnames(df)[which(colnames(df) == "response")] <- "RNAvcQualityScore"
		df[get_tstv(df$MutationType) == "TS" & df$RNAvcQualityScore >= 0.25,]$RNAvcIsGood <- TRUE
		df[get_tstv(df$MutationType) == "TV" & df$RNAvcQualityScore >= decision.boundry,]$RNAvcIsGood <- TRUE
	} else {
		if (!("RNAvcScore" %in% colnames(df))) {
			df$RNAvcScore = 1
		}
		if (!("RNAvcClass" %in% colnames(df))) {
			df$RNAvcClass <- factor("SNV",levels=c("Edit","SNV"))
		}
		df$RNAvcScore = pmin(df$RNAvcScore,df$response)
		df[df$RNAvcScore >= decision.boundry,]$RNAvcClass <- "SNV"
		df[df$RNAvcScore < decision.boundry,]$RNAvcClass <- "Edit"
	}
...
test.RNAvc <- function (gbm.model,test, response=FALSE,class="AI") { 
	library(gbm)
	library(caret)

	test <- predict.RNAvc(gbm.model,test,class=class)
	...
	if (class == "error") {
		...
		print(confusionMatrix(test2$RNAvcIsGood,test2$y,positive="TRUE"))
	}
	else { 
		test2 <- only_edit(test,class=class,keep.MutationType=TRUE)
		test2$SiteClass <- droplevels(test2$SiteClass)
		print(confusionMatrix(test2$RNAvcClass,test2$SiteClass,positive="SNV"))
	}
...
cleanFeatures <- function (df,sample=NULL) {
	...
	#df$RNAvc_score <- (df$MedianDistFromEnd + df$MaxDistFromEnd)
...
#		"##INFO<ID=SNPClust,Number=1,Type=Integer,Description=\"Number of SNPs in 10 bp window.\">",
		"##INFO<ID=RNAvcIsGood,Number=1,Type=Character,Description=\"Classification of variant as error.\">",
		"##INFO<ID=RNAvcQualityScore,Number=1,Type=Character,Description=\"Confidence of classification of variant as an error (0 = SNV, 1=Edit site).\">",
		"##INFO<ID=RNAvcClass,Number=1,Type=Character,Description=\"Classification of variant as edit site, genomic SNV, or Not predicted.\">",
		"##INFO<ID=RNAvcScore,Number=1,Type=Character,Description=\"Confidence of classification (0 = SNV, 1=Edit site).\">"
	)

	header <- c(header,RNAvc_text,lastline)
	...
	RNAvc <- data.frame( RNAvcQualityScore=rep(NA,nrow(df)), RNAvcIsGood=factor(rep("NotPredicted",nrow(df)),levels=c("NotPredicted","TRUE","FALSE")), RNAvcScore=rep(NA,nrow(df)), RNAvcClass=factor(rep("NotPredicted",nrow(df)),levels=c("NotPredicted","Edit","SNV")))
	rownames(RNAvc) <- rownames(df)
	...
	index <- pmatch(rownames(prediction),rownames(RNAvc))
	RNAvc[index,1] <- signif(prediction$RNAvcQualityScore,digits=3)
	RNAvc[index,2] <- prediction$RNAvcIsGood	
	RNAvc[index,3] <- signif(prediction$RNAvcScore,digits=3)
	RNAvc[index,4] <- prediction$RNAvcClass
...
#	index <- pmatch(names(append.score),rownames(prediction))
	...
	df[,8] <- paste(df[,8],";RNAvcIsGood=",RNAvc$RNAvcIsGood,sep="")
	df[,8] <- paste(df[,8],";RNAvcQualityScore=",RNAvc$RNAvcQualityScore,sep="")
	df[,8] <- paste(df[,8],";RNAvcClass=",RNAvc$RNAvcClass,sep="")
	df[,8] <- paste(df[,8],";RNAvcScore=",RNAvc$RNAvcScore,sep="")
...
predict.models <- function(df,error=NULL,ai=NULL,cu=NULL) {
	if (!is.null(error)) {
		df <- predict.RNAvc(error,df,class="error")
	}
	if (!is.null(ai)) {
		df<- predict.RNAvc(ai,df,class="AI")
	}
	if (!is.null(cu)) {
		df<- predict.RNAvc(cu,df,class="CU")
	}
...
test.models <- function(df,error=NULL,ai=NULL,cu=NULL) {
	if (!is.null(error)) {
		df <- test.RNAvc(error,df,class="error",response=TRUE)
	}
	if (!is.null(ai)) {
		df<- test.RNAvc(ai,df,class="AI",response=TRUE)
	}
	if (!is.null(cu)) {
		df<- test.RNAvc(cu,df,class="CU",response=TRUE)
	}
	#res <- rbind(res.err,res.ai,res.cu)
