...
if ($train || $predict) {
	my $get_features = "$RNAvc_dir/GetFeatures.pl -c $CONFIG ";
	...
	my $r_cmd = "R --file='$RNAvc_dir/RNAvc.R' CONFIG=$CONFIG $HEADER.features ";
...
RNAvc.pl - calls RNA-seq based variants from BAM files. 
...
RNAvc.pl -c <configuration file> [options] [BAM1 BAM2 ...]
